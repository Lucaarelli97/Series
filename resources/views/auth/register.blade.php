        ...
        <div class="w-full">
            <section class="flex flex-col break-words bg-black sm:border-1 sm:rounded-md sm:shadow-sm sm:shadow-lg">
                ...
                <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8" method="POST"
                    ...
                    <div class="flex flex-wrap">
                        <button type="submit"
                            class="w-full select-none font-bold whitespace-no-wrap p-3 rounded-lg text-base leading-normal no-underline text-white bg-red-800 hover:bg-blue-700 sm:py-4">
                            {{ __('Register') }}
                        </button>

                        <p class="w-full text-xs text-center text-white my-6 sm:text-sm sm:my-8">
                            {{ __('Already have an account?') }}
                            <a class="text-red-800 hover:text-blue-700 no-underline hover:underline" href="{{ route('login') }}">
                                {{ __('Login') }}
                            </a>
                        </p>
                    </div>
