...
<form class="w-full max-w-lg border-4 " method="POST" action="{{ route('contacta.store') }}">
    @csrf
     <h1 class="font-semibold py-5 text-blue mb-10 bg-red-800 text-white px-5"> Formulario </h1>
    
    <div class="flex flex-wrap -mt-10">
        <div class="w-full px-5">
            <label class="block uppercase tracking-wide text-white text-xs font-bold my-5" for="name">
                {{ __("Nombre") }}
            </label>
            ...
            <p class="text-red-700 text-xs italic -my-3">{{ __("Nombre del usuario") }}</p>
            @error("name")
    ...
    <div class="flex flex-wrap mt-3 ">
        <div class="w-full px-5">
            <label class="block uppercase tracking-wide text-white text-xs font-bold my-5" for="email">
                {{ __("Email") }}
            </label>
            ...
            <p class="text-red-700 text-xs italic -my-3">{{ __("Email de contacto") }}</p>
            @error("email")
            <div class="border border-red-400 rounded-b bg-red-100 mt-1 px-4 py-3 text-red-700">
                {{ $message }}
            </div>
    ...
    <div class="flex flex-wrap mt-3">
        <div class="w-full px-5">
            <label class="block uppercase tracking-wide text-white text-xs font-bold my-5" for="mensaje">
                {{ __("Mensaje") }}
            </label>
            ...
            <p class="text-red-700 text-xs italic -my-3">{{ __("Escribe tu mensaje") }}</p>
            @error("mensaje")
            <div class="border border-red-400 rounded-b bg-red-100 mt-1 px-4 py-3 text-red-700">
                {{ $message }}
            </div>
    ...
    <div class="md:flex md:items-center">
        <div class="md:w-1/3">
            <button class="shadow bg-red-600 mt-5 hover:bg-red-900 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="submit">
                {{ 'Enviar mensaje' }}
            </button>
        </div>
    </div>
